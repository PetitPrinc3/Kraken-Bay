        ...
        <div onBlur={closeSearch}>
            <div className="flex flex-col gap-2">
                <div onClick={() => { window.screen.width < 800 ? router.push("/search") : openSearch(pathname); searchInput.current.focus() }} className="text-gray-200 flex flex-row gap-2 items-center hover:text-gray-300 cursor-pointer">
                    <BsSearch />
                    <div className={`hidden md:inline-block ${isOpen ? "w-[30vw] md:w-[15vw]" : "w-0"} overflow-hidden transition-all ease-in-out duration-200`}>
                        <input
                            onChange={e => { search(e.currentTarget.value) }}
                            ref={searchInput}
                            defaultValue={searchText}
                            className={`rounded-md transition-all duration-500 focus:outline-none hidden md:inline-block w-[30vw] md:w-[15vw] ${isOpen ? "px-2 bg-zinc-900 border-2 border-zinc-400" : "px-0 border-none"}`}
