...
const Switch: React.FC<SwitchProps> = ({ state = false, disabled = false, values = ["Off", "On"], onChange = () => { } }) => {

    const [isOn, toggle] = useState(state)

    return (
        <div className="relative w-full h-full md:py-1">
            <div onClick={() => { if (!disabled) { toggle(!isOn); onChange() } }} className="relative w-full grid grid-cols-2 items-cente mx-auto text-slate-800 font-light text-xs text-center">
                <div className={`w-full relative flex items-center ${disabled ? "cursor-not-allowed" : "cursor-pointer"} p-0.5 py-1 mx-auto justify-center rounded-full z-[1] ${!isOn ? "font-semibold" : "text-slate-500"} transition-all duration-200`}>
                    {values[0]}
                </div>
                <div className={`w-full relative flex items-center ${disabled ? "cursor-not-allowed" : "cursor-pointer"} pr-0.5 py-1 mx-auto justify-center rounded-full z-[1] ${isOn ? "font-semibold" : "text-slate-500"} transition-all duration-200`}>
                    {values[1]}
                </div>
