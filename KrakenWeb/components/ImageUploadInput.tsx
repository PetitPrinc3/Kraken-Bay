...
const ImageUploadInput: React.FC<ImageInputProps> = ({
    ...
    const imageChange = async (event: any) => {
        if (event.target.files && event.target.files.length > 0) {
            ...
            if (name == "Thumbnail") uploadProps.setThumb(imageFile)
            if (name == "Poster") uploadProps.setPoster(imageFile)
    ...
    return (
        <div className="flex flex-row items-center w-full">
            <div className={`${!image ? "hidden" : "flex flex-row gapp-4 items-center w-full rounded-md bg-neutral-600 border-r-2 border-r-green-500 md:pr-4"}`} onClick={removeImage}>
                <img className="rounded-md h-14 max-w-[80%] mr-auto cursor-default" src={image || "/Assets/Images/default_profile.png"} alt="" />
                <p className="hidden sm:inline-block md:hidden ml-4 mr-0 text-neutral-800 truncate text-ellipsis">{imageName}</p>
            ...
            <div className={`${image ? "hidden" : "relative w-full cursor-pointer"}`}>
                <input id="poster-file" type="file" accept="image/*" onChange={(e: any) => { imageChange(e) }} ref={thumbRef} className="hidden" />
